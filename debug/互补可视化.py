--- START OF FILE debug/advanced_analyzer.py ---
# -*- coding: utf-8 -*-
"""
Advanced Observable AHRS Analyzer (v1.0)

This script is specifically designed to analyze and visualize the internal
workings of "observable" AHRS algorithms like AccelAhrs.

It generates two key plots:
1. Scored Error Plot: Compares the actual attitude error against the
   algorithm's predicted uncertainty bounds (3-sigma).
2. Internal Diagnostics Plot: Visualizes the algorithm's decision-making
...
ALGORITHM_TO_ANALYZE = 'AccelAhrs'
SEQUENCE_TO_ANALYZE = 'V1_01_easy' # 'V2_03_difficult' is also a good test case
RESULTS_BASE_PATH = r'../results' # Relative path to results
...
def read_ahrs_telemetry_csv(filepath):
    ...
    try:
        ...
        if 'accel_innovation_x' in header:
            telemetry['innovation'] = data[:, header.index('accel_innovation_x'):header.index('accel_innovation_y')+1]
        if 'correction_gain_roll' in header:
            telemetry['gain'] = data[:, header.index('correction_gain_roll'):header.index('correction_gain_pitch')+1]
        if 'is_correction_active' in header:
            telemetry['correction_active'] = data[:, header.index('is_correction_active')]
        if 'raw_accel_x' in header: # Need to add this to telemetry export if we want to plot it
...
def plot_scored_error(time_axis, errors_deg, p_std_dev_deg, title_info):
...
def plot_internal_diagnostics(time_axis, telemetry, raw_accel, title_info):
    ...
    fig, axes = plt.subplots(3, 1, figsize=(18, 12), sharex=True)
    fig.suptitle(f'Internal Algorithm Diagnostics\n{title_info}', fontsize=16)

    # Subplot 1: Accelerometer Magnitude and Correction Trigger
    accel_mag = np.linalg.norm(raw_accel, axis=1)
    threshold_val = 0.1 # This should ideally be read from config, hardcoded for now
    ...
    ax1.axhline(GROUND_TRUTH_GRAVITY * (1 + threshold_val), color='red', linestyle=':', label=f'Correction Threshold (Upper)')
    ax1.axhline(GROUND_TRUTH_GRAVITY * (1 - threshold_val), color='red', linestyle=':', label=f'Correction Threshold (Lower)')
    ...
    ax2 = axes[1]
    ax2.plot(time_axis, telemetry['innovation'][:, 0], label='Innovation (Roll Error)', color='coral')
    ax2.plot(time_axis, telemetry['innovation'][:, 1], label='Innovation (Pitch Error)', color='mediumseagreen')
    ax2.set_ylabel('Innovation Signal [rad]')
    ax2.legend()
...
def main():
    ...
    # --- 4. Generate Plots ---
    plot_scored_error(time_axis, error_eul_deg, p_std_dev_deg, title_info)
...
if __name__ == '__main__':
    main()
--- END OF FILE debug/advanced_analyzer.py ---